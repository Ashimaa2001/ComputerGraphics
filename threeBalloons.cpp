...
void idle() {
    ...
    balloon_X[0] += balloonSpeedX[0];
    balloon_Y[0] += balloonSpeedY[0];
    balloon_X[1] += balloonSpeedX[1];
    balloon_Y[1] += balloonSpeedY[1];
    balloon_X[2] += balloonSpeedX[2];
    balloon_Y[2] += balloonSpeedY[2];

    if (balloon_X[0] + balloonRadius[0] > 1000.0f || balloon_X[0] - balloonRadius[0] < -1000.0f) {
        balloonSpeedX[0] = -balloonSpeedX[0];
    }

    if (balloon_Y[0] + balloonRadius[0] > 700.0f || balloon_Y[0] - balloonRadius[0] < -700.0f) {
        balloonSpeedY[0] = -balloonSpeedY[0];
    }

    if (balloon_X[1] + balloonRadius[1] > 1000.0f || balloon_X[1] - balloonRadius[1] < -1000.0f) {
        balloonSpeedX[1] = -balloonSpeedX[1];
    }

    if (balloon_Y[1] + balloonRadius[1] > 700.0f || balloon_Y[1] - balloonRadius[1] < -700.0f) {
        balloonSpeedY[1] = -balloonSpeedY[1];
    }

    if (balloon_X[2] + balloonRadius[2] > 1000.0f || balloon_X[2] - balloonRadius[2] < -1000.0f) {
        balloonSpeedX[2] = -balloonSpeedX[2];
    }

    if (balloon_Y[2] + balloonRadius[2] > 700.0f || balloon_Y[2] - balloonRadius[2] < -700.0f) {
        balloonSpeedY[2] = -balloonSpeedY[2];
    }
...
void gameStartDisplay()
{
    ...
    drawBall(balloon_X[1], balloon_Y[1], balloonRadius[1]);
    drawBall(balloon_X[2], balloon_Y[2], balloonRadius[2]);
    checkPlayerContact(balloon_X[0], balloon_Y[0]);
    checkPlayerContact(balloon_X[1], balloon_Y[1]);
    checkPlayerContact(balloon_X[2], balloon_Y[2]);
    if(laser) {
        DrawLaser(xOne, yOne);
